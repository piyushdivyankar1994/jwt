...
void uint64_to_big_endian(uint64_t val, uint8_t *bytes) {
  bytes[0] = (val >> 56) & 0xFF;
  bytes[1] = (val >> 48) & 0xFF;
  bytes[2] = (val >> 40) & 0xFF;
  bytes[3] = (val >> 32) & 0xFF;
...
uint8_t *sha256_pad_message(const char *message, size_t msg_len,
                            size_t *padded_len) {
...
uint32_t rightrotate(uint32_t inp, uint32_t rotate) {
  rotate = rotate % 32;
  if (rotate == 0)
    return inp;
...
char *sha256_encode(const char *input, const size_t len) {
  ...
  // 4. FIXED: Convert the 8 final 32-bit integers into a 64-character hex
  // string
  ...
  for (int i = 0; i < 8; i++) {
    sprintf(&hex_output[i * 8], "%08x", h[i]);
  }
